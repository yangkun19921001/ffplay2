 ...
 * @param q
 */
void ff_packet_queue_destroy(PacketQueue *q) {
...
void ff_packet_queue_start(PacketQueue *q) {
    SDL_LockMutex(q->mutex);
    q->abort_request = 0;
    ff_packet_queue_put_private(q, q->flush_pkt); //这里放入了一个flush_pkt
    SDL_UnlockMutex(q->mutex);
}

/**
 * 从队列中获取一个包，如果队列为空 ，可以进行阻塞等待
 * @param q
 * @param pkt
 * @param block
 * @param serial
 ...
 * @return
 */
int ff_packet_queue_put_nullpacket(struct PacketQueue *q, int stream_index) {
...
int ff_packet_queue_put_private(struct PacketQueue *q, AVPacket *pkt) {
    ...
    pkt1->next = NULL;
    if (pkt == q->flush_pkt)//如果放入的是flush_pkt，需要增加队列的播放序列号，以区分不连续的两段数据
    {
        q->serial++;
        printf("ff_packet_queue_put_private q->serial = %d\n", q->serial);
    }
